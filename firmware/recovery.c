...
static void recovery_done(void) {
	...
	if (!enforce_wordlist || mnemonic_check(new_mnemonic)) {
		...
		if (!dry_run) {
			...
			if (same_mnemonic) {
				layoutDialog(&bmp_icon_ok, NULL, _("Confirm"), NULL,
					"The mnemonic is",
					"valid and matches",
					"existing one.", NULL, NULL, NULL);
				protectButton(ButtonRequestType_ButtonRequest_Other, true);
				fsm_sendSuccess("Mnemonic is valid and matches existing one");
			} else {
				layoutDialog(&bmp_icon_error, NULL, _("Confirm"), NULL,
					"The mnemonic is",
					"valid but doesn't",
					"match existing one.", NULL, NULL, NULL);
				protectButton(ButtonRequestType_ButtonRequest_Other, true);
				fsm_sendFailure(FailureType_Failure_DataError,
					"Mnemonic is valid but doesn't match existing one");
			}
		}
	} else {
		...
		if (!dry_run) {
			...
			layoutDialog(&bmp_icon_error, NULL, _("Confirm"), NULL,
				"The mnemonic is", "invalid!", NULL, NULL, NULL, NULL);
			protectButton(ButtonRequestType_ButtonRequest_Other, true);
		}
